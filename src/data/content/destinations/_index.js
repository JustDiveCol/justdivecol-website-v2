...
export const publishedDestinations = allDestinations.filter((dest) => dest.status === 'published');
...
export const getDestinationsWithUpcomingTrips = () => {
  ...
  return publishedDestinations.map((destination) => {
    ...
    const associatedTrips = publishedExperiences
      .filter(
        (trip) => trip.destinationId === destination.id && new Date(trip.details.endDate) >= today
      )
