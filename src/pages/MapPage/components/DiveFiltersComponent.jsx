      ...
      <div className="flex flex-col gap-6 mb-6">
        ...
        {destinationOptions.length > 0 && (
          <div className="w-full">
            <span className="text-sm md:text-base lg:text-lg block font-semibold text-brand-neutral mb-2">
              {t(SHARED_TRANSLATION_KEYS.DESTINATION_LABEL)}
            </span>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-between">
          <div className="flex flex-col sm:flex-row flex-grow gap-6">
            ...
            <div className="w-full sm:w-1/2">
              <span className="text-sm md:text-base lg:text-lg block font-semibold text-brand-neutral mb-2">
                {t(SHARED_TRANSLATION_KEYS.TYPE_LABEL)}
              </span>
              <div className="flex flex-wrap gap-2 min-h-[26px] items-center">
                {typeFilterOptions.length > 1 ? (
                  ...
                  <span className="text-sm text-brand-neutral/70 italic">
                    {t(SHARED_TRANSLATION_KEYS.NO_OPTIONS_AVAILABLE_LABEL)}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center justify-center sm:justify-end w-full sm:w-auto">
            ...
            >
              {t(SHARED_TRANSLATION_KEYS.CLEAR_FILTERS_LABEL)}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start mb-4">
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder={t(SHARED_TRANSLATION_KEYS.SEARCH_PLACEHOLDER_TEXT)}
          ...
          {searchQuery && (
            <button
              ...
              aria-label={t(SHARED_TRANSLATION_KEYS.CLEAR_FILTERS_LABEL)}
            >
              ✕
            </button>
          )}
        ...
        <div className="flex items-center text-sm sm:justify-end sm:text-right h-full">
          {filteredSites.length === 0 ? (
            <div className="text-brand-neutral text-md">
              {t(SHARED_TRANSLATION_KEYS.NO_MATCHING_SITES_LABEL)}
            </div>
          ) : (
            <div className="text-brand-neutral text-md">
              {t(SHARED_TRANSLATION_KEYS.SHOWING_SITES_COUNT_FULL_TEXT, {
